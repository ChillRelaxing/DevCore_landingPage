  ...
  <div class="min-h-screen transition-colors duration-300 dark:bg-slate-950 dark:text-slate-100 bg-slate-50 text-slate-900 font-sans">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-18 items-center">
          <div class="flex items-center gap-2 cursor-pointer group" @click="scrollToSection('hero')">
            <div class="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-black text-xl group-hover:rotate-12 transition-transform">
              D
            </div>
            <span class="font-bold text-2xl tracking-tighter dark:text-white text-slate-900">DevCore</span>
          </div>

          <!-- Desktop Links -->
          <div class="hidden lg:flex space-x-6 items-center">
            <button v-for="link in navLinks" :key="link.id" @click="scrollToSection(link.id)" class="text-sm font-medium dark:text-slate-400 text-slate-600 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
              {{ link.name }}
            </button>
            ...
            <button @click="scrollToSection('contacto')" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-full font-semibold transition-all shadow-lg shadow-indigo-500/25 text-sm">
              Contactar
            </button>
          </div>
      ...
      <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-4" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 -translate-y-4">
        <div v-if="mobileMenuOpen" class="lg:hidden dark:bg-slate-950 bg-white border-b dark:border-slate-800 border-slate-200">
          <div class="px-4 pt-4 pb-6 space-y-2">
            <button v-for="link in navLinks" :key="link.id" @click="scrollToSection(link.id)" class="block w-full text-left px-4 py-3 rounded-xl text-lg font-medium hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
              {{ link.name }}
            </button>
            <div class="pt-4 px-4">
              <button @click="scrollToSection('contacto')" class="w-full bg-indigo-600 text-white px-4 py-4 rounded-xl font-bold">
                Contactar
              </button>
            </div>
          </div>
    ...
    <header id="hero" class="relative pt-32 pb-20 lg:pt-52 lg:pb-40 overflow-hidden">
      <div class="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-full -z-10 opacity-30">
        <div class="absolute top-0 right-0 w-96 h-96 bg-indigo-500 rounded-full blur-[120px]"></div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center lg:text-left flex flex-col lg:flex-row items-center gap-16">
        <div class="flex-1">
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-500 text-xs font-bold mb-8 border border-indigo-500/20">
          ...
          <h1 class="text-5xl lg:text-7xl font-black tracking-tight mb-6 leading-[1.1]">
            Ingeniería de <br />
            <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-blue-400">
              Software de Élite
            </span>
          </h1>
        ...
        <div class="flex-1 relative">
          ...
          <div class="absolute -top-10 -right-10 w-40 h-40 bg-indigo-600/20 rounded-full blur-3xl -z-10"></div>
    ...
    <section id="servicios" class="py-24 relative">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-20">
          <h2 class="text-indigo-500 font-bold uppercase tracking-widest text-sm mb-3">Servicios</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div v-for="service in services" :key="service.title" class="p-10 rounded-3xl dark:bg-slate-900/50 bg-white border dark:border-slate-800 border-slate-200 hover:border-indigo-500/50 transition-all group">
    ...
    <!-- Industry Section -->
    <section id="industria" class="py-24 dark:bg-slate-900 bg-indigo-600 text-white overflow-hidden">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col lg:flex-row items-center gap-16">
          <div class="flex-1">
            <h2 class="text-indigo-300 font-bold uppercase tracking-widest text-sm mb-3">Industrias</h2>
            <p class="text-4xl font-black mb-6 leading-tight">Expertos en mercados verticales de alta demanda</p>
            <p class="text-indigo-100 mb-8 text-lg opacity-80">
          ...
          <div class="flex-1 flex justify-center">
            <div class="grid grid-cols-2 gap-6 rotate-12 scale-110 opacity-20 lg:opacity-100">
               ...
               <div class="w-40 h-40 bg-indigo-300 rounded-3xl translate-y-10"></div>
               <div class="w-40 h-40 bg-indigo-400 rounded-3xl -translate-y-10"></div>
    ...
    <section id="nosotros" class="py-24">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col lg:flex-row gap-20 items-center">
          ...
          <div class="flex-1">
            <h2 class="text-indigo-500 font-bold uppercase tracking-widest text-sm mb-3">Sobre Nosotros</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
              <div class="p-6 rounded-2xl bg-indigo-500/5 border border-indigo-500/10">
                <h4 class="text-indigo-600 dark:text-indigo-400 font-bold mb-2 flex items-center gap-2">
            ...
            <ul class="space-y-4">
              <li class="flex items-start gap-4">
                <div class="w-6 h-6 rounded-full bg-indigo-500 flex-shrink-0 flex items-center justify-center mt-1">
    ...
    <section id="proyectos" class="py-24 dark:bg-slate-900/30 bg-slate-100">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row justify-between items-end mb-16">
          <div>
            <h2 class="text-indigo-500 font-bold uppercase tracking-widest text-sm mb-3">Portafolio</h2>
            <p class="text-4xl font-black">Proyectos Destacados</p>
          </div>
          <button class="hidden md:block text-indigo-500 font-bold hover:underline">Ver todos los casos de éxito →</button>
        </div>
    ...
    <section id="carreras" class="py-24">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div class="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-[3rem] p-12 lg:p-20 text-white shadow-2xl relative overflow-hidden">
          <div class="absolute -top-20 -right-20 w-64 h-64 bg-white/10 rounded-full blur-3xl"></div>
          <div class="relative z-10">
            <h2 class="text-4xl lg:text-5xl font-black mb-8 leading-tight">¿Eres un apasionado del código?</h2>
            <p class="text-xl text-indigo-100 mb-10 max-w-2xl mx-auto opacity-90">
            ...
            <button class="bg-white text-indigo-600 px-10 py-4 rounded-2xl font-black hover:bg-slate-100 transition shadow-xl">
    ...
    <section id="perspectivas" class="py-24">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
          <h2 class="text-indigo-500 font-bold uppercase tracking-widest text-sm mb-3">Perspectivas</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          <div class="group">
            ...
            <span class="text-indigo-500 text-sm font-bold uppercase">Tecnología</span>
            <h4 class="text-xl font-bold mt-2 mb-4 group-hover:text-indigo-500 transition-colors">El impacto de WebAssembly en el desarrollo frontend moderno</h4>
          ...
          <div class="group">
            ...
            <span class="text-indigo-500 text-sm font-bold uppercase">Cultura</span>
            <h4 class="text-xl font-bold mt-2 mb-4 group-hover:text-indigo-500 transition-colors">Cómo mantenemos la cohesión en equipos 100% remotos</h4>
            <p class="dark:text-slate-400 text-slate-600 text-sm leading-relaxed">
          ...
          <div class="group">
            ...
            <span class="text-indigo-500 text-sm font-bold uppercase">Tutorial</span>
            <h4 class="text-xl font-bold mt-2 mb-4 group-hover:text-indigo-500 transition-colors">Escalando APIs con NestJS y Microservicios</h4>
            <p class="dark:text-slate-400 text-slate-600 text-sm leading-relaxed">
    ...
    <section id="contacto" class="py-24 relative overflow-hidden">
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl h-full -z-10 opacity-20">
        <div class="absolute top-0 right-0 w-80 h-80 bg-indigo-500 rounded-full blur-[100px] animate-pulse"></div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col md:flex-row">
          <div class="flex-1 bg-indigo-600 p-12 text-white">
          ...
          <div class="flex-[1.5] p-12">
            <form v-if="!showSuccess" @submit.prevent="submitContactForm" class="space-y-6">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input v-model="contactForm.name" required type="text" placeholder="Tu nombre" class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-5 py-4 focus:ring-2 focus:ring-indigo-500 transition-all" />
                </div>
                <div>
                  <label class="block text-sm font-bold mb-2 ml-1">Email</label>
                  <input v-model="contactForm.email" required type="email" placeholder="tu@email.com" class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-5 py-4 focus:ring-2 focus:ring-indigo-500 transition-all" />
                </div>
              </div>
              <div>
                <label class="block text-sm font-bold mb-2 ml-1">Mensaje</label>
                <textarea v-model="contactForm.message" required rows="4" placeholder="¿En qué podemos ayudarte?" class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-5 py-4 focus:ring-2 focus:ring-indigo-500 transition-all resize-none"></textarea>
              </div>
              <button :disabled="isSubmitting" type="submit" class="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black text-lg hover:bg-indigo-700 transition shadow-xl shadow-indigo-500/30 flex items-center justify-center gap-3 disabled:opacity-70">
            ...
            <div v-else class="h-full flex flex-col items-center justify-center text-center py-12">
              ...
              <button @click="showSuccess = false" class="mt-8 text-indigo-600 font-bold hover:underline">Enviar otro mensaje</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          <div class="col-span-1 lg:col-span-1">
            <div class="flex items-center gap-2 mb-6">
              ...
              <span class="font-bold text-xl tracking-tighter">DevCore</span>
            </div>
